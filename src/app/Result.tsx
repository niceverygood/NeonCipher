    ...
    <Screen>
      <div
        style={{
          ...
          gap: 18,
          background: victory
            ? 'radial-gradient(120% 70% at 50% 0%,rgba(0,234,255,.12),transparent 60%)'
            : 'radial-gradient(120% 70% at 50% 0%,rgba(255,42,109,.12),transparent 60%)',
        }}
      >
        <motion.div
          ...
          data-t={victory ? 'CLEAR' : 'FAILED'}
          style={{
            fontSize: 52,
            fontWeight: 700,
            color: victory ? '#f3f6ff' : 'var(--mag)',
            textShadow: victory ? '0 0 28px rgba(0,234,255,.5)' : '0 0 28px rgba(255,42,109,.5)',
          }}
        >
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {[0, 1, 2].map((i) => (
            <motion.span
              ...
              animate={{ scale: i < result.stars ? 1.1 : 0.8, rotate: 0 }}
              transition={{ delay: 0.3 + i * 0.18, type: 'spring', stiffness: 260 }}
              style={{
                fontSize: 40,
                color: i < result.stars ? 'var(--surge)' : 'var(--dim)',
                textShadow: i < result.stars ? '0 0 16px var(--surge)' : 'none',
              }}
            >
              ★
            </motion.span>
